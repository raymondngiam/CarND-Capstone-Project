    ...
    def traffic_cb(self, msg):
        ...
        for light in self.lights:
            ...
            if True:
                ...
                if (is_logging):
                    rospy.loginfo("%s: found state[%s] @ %s", self.name, \
                              light.state, \
                              stop_wp_index)
                if light.state == TrafficLight.RED:
                    if  stop_wp_index > car_wp_index and stop_wp_index < closest_stop_index:
                        closest_stop_index = stop_wp_index

        if (is_logging):
            rospy.loginfo("%s: closest stop @ %s", self.name, closest_stop_index)
...
if __name__ == '__main__':
    try:
        ...
        rospy.logerr('Could not start dummy detector node.')
